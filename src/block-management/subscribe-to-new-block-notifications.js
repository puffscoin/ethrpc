...
// try to subscribe to new blocks using puffs_subscribe, fall back to polling if not available
function subscribeToNewBlockNotifications(subscriptionFailedCallback) {
  return function (dispatch, getState) {
    ...
    dispatch(puffs.subscribe(["newHeads", {}], function (err, newHeadsSubscriptionID) {
      ...
      if (getState().newHeadsSubscription.id === undefined) {
        dispatch(puffs.unsubscribe(newHeadsSubscriptionID, logError));
      } else {
